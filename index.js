...
const CONTRACT_ADDRESS = "0x91a99F273Ad59ba1F5FeB6ca95C93903A70e94c7";
...
app.get("/view-all-fir-investigate", async (req, res) => {
  try {
    ...
    const entries = await GoogleForm.methods
      .viewAllFIRInvestigator()
      .call({ from });
    res.status(200).send(safeJSON(entries));
  } catch (error) {
    console.error(error);
...
app.get("/view-fir", async (req, res) => {
  try {
    ...
    if (!from || !fir_id) {
      return res
        .status(400)
        .json({ message: "'from' and 'fir_id' are required" });
    }
  ...
  try {
    ...
    if (!fir_id || !message || !sender_address) {
      return res.status(400).json({
        message: "'fir_id', 'message', and 'sender_address' are required",
      });
    }
    ...
    res
      .status(200)
      .send({ message: "FIR closed successfully", result: safeJSON(result) });
  } catch (error) {
    console.error(error);
...
app.post("/investigate-fir", async (req, res) => {
  try {
    ...
    const result = await GoogleForm.methods
      .markAsInvestigated(fir_id, message)
      .send({
        from: sender_address,
        gas: 6721975,
        gasPrice: 20000000000,
    ...
    res.status(200).send({
      message: "FIR marked as investigated",
...
app.post("/new-fir", async (req, res) => {
  try {
    ...
    if (!message || !sender_address) {
      return res
        .status(400)
        .json({ message: "'message' and 'sender_address' are required" });
    }
    ...
    res.status(200).send({
      message: "New FIR created successfully",
      result: safeJSON(result),
    });
  } catch (error) {
    console.error(error);
...
app.post("/validate-fir", async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "'fir_id', 'isApproved', 'message', and 'sender_address' are required",
      });
    }

    const result = await GoogleForm.methods
      .updateFIRStatus(fir_id, isApproved, message)
      .send({
        from: sender_address,
        gas: 6721975,
        gasPrice: 20000000000,
    ...
    res.status(200).send({
      message: "FIR validated successfully",
      result: safeJSON(result),
    });
  } catch (error) {
    console.error(error);
